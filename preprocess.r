...
# Marca se fez a prova no mesmo estado que se inscreveu
...
# Remove algumas informaçẽos sobre identificadores e localização
...
dat <- subset(dat, select=-c(ID_DEPENDENCIA_ADM, ID_LOCALIZACAO, SIT_FUNC))
...
# Remove estado civil e tipo de ensino (pouca variação ou relevancia)
dat <- subset(dat, select=-c(TP_ESTADO_CIVIL, IN_TP_ENSINO, ANO_CONCLUIU, TP_ESCOLA))
